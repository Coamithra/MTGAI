...
- **`GET /api/runtime/state`**: aggregator endpoint every page hits on mount. Returns `{active_set, ai_lock, active_runs, pipeline, theme}`. `active_set` resolves from `?set_code=` -> most-recent on-disk `pipeline-state.json` -> most-recent `theme.json` -> `MTGAI_REVIEW_SET` env -> `"ASD"`.
