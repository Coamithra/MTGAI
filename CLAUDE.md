...
- **Flash attention** is on by default in `llama-server` for architectures that support it (no flag needed). Per-model KV-cache quantization happens at server launch via `--cache-type-k` / `--cache-type-v` flags, which llmfacade passes through automatically when `cache_type_k` / `cache_type_v` are set on the model entry in `models.toml`. No global env var; per-server, per-model.
- **Default 26B config**: `gemma4-26b-vram-dynamic` in `models.toml` is the long-context winner (per TC-1f) — Vlad IQ4_XS GGUF + `cache_type_k = "q8_0"` + `cache_type_v = "q8_0"` + `n_gpu_layers = -1`. q8_0 KV trims cache from ~3.4 GB (f16) to ~1.7 GB at 128K, freeing layers for GPU placement. Per-model declaration replaces Ollama's global `OLLAMA_KV_CACHE_TYPE` env var.
