...
- **API**: `try_acquire(name, log_path=None)` / `release()`, `hold(name, log_path=None)` context manager (yields True on success, False if busy), `is_running()`, `current_action()`, `request_cancel()`, `is_cancelled()`, `cancel_event()`, `update_log_path(path)` for late-binding the per-run log dir, `reset_for_tests()` (test-only).
