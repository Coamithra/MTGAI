...
Making MTGAI reusable for any set, not just ASD. Say "continue toolchain buildout" to resume. Shipped: TC-1 (theme extraction), TC-2 (mechanic gen; TC-5 pointed-questions template folded in), TC-3 (archetype gen — `mtgai/generation/archetype_generator.py` + the `archetypes` stage, between `mechanics` and `skeleton`; writes `<asset>/archetypes.json`, one `DraftArchetype`-shaped entry per color pair). Remaining work tracked on Trello: TC-4 (visual refs) → TC-6 (prompts module — make it consume `archetypes.json`; the override param is already threaded) → TC-7 (skeleton + configure integration).
