...
- [x] **TC-1b**: Ollama native API migration — rewrote `_generate_ollama()` to use native `/api/chat` endpoint. Now passes `num_ctx` (from model registry) and `num_predict` per-request. Connection retry via `requests` instead of `openai` SDK. Handles tool args as string or dict.
- [x] **TC-1c**: Token counting and truncation detection — `token_utils.py` with tiktoken cl100k_base counting, pre-call `ContextOverflowError`, post-call `InputTruncatedError`/`OutputTruncatedError`. `ollama_debug.py` log scanner for server.log (truncation, OOM, CUDA errors, generic warn/error). `MTGAI_DEBUG=1` env var enables post-call log scanning. Theme extractor now uses shared token counter. NOTE: Ollama PR #12030 adds native `/api/tokenize` for exact model-specific counts - upgrade when merged.
...
- [ ] **TC-2**: Mechanic generation stage — first entry in `STAGE_RUNNERS`, runs after theme extraction and before skeleton. Generates 6 candidates from `theme.json` + `set_params.mechanic_count`, pauses for human review (`paused_for_review`), user picks `mechanic_count` and edits inline, save handler writes `approved.json` + `evergreen-keywords.json` + `pointed-questions.json` (canonical 9-question template substituted, closes TC-5) + `functional-tags.json` (empty stub). Standard stage tab via `wizard_stage.js` with bespoke candidates body. Spec: `plans/phase-tc2-mechanic-gen.md`. → [Trello](https://trello.com/c/yw02hjGi)
  ...
  - [ ] **TC-2b**: Add `run_mechanics` runner + register in `STAGE_RUNNERS` (first entry) + stage artifact clearer.
  - [ ] **TC-2c**: `wizard_mechanics.js` — bespoke candidates strip body inside the standard stage shell.
  - [ ] **TC-2d**: `/api/wizard/mechanics/{refresh-card,refresh-all,save}` endpoints (kickoff/cancel reuse standard stage endpoints).
  - [ ] **TC-2e**: Template assets — `pipeline/templates/pointed_questions.json`, `mtg_known_keywords.json`, evergreen-keywords default table.
  - [ ] **TC-2f**: ASD migration smoke — open existing project, mechanics stage shows `completed`, edit via cascade clears skeleton + downstream.
...
- [ ] **TC-5**: Pointed questions template — folded into TC-2 save handler. Canonical 9-question template at `pipeline/templates/pointed_questions.json`, mechanic name substitution at save time. → [Trello](https://trello.com/c/RHKh2z6D)
