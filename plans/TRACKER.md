...
- [ ] **TC-1e**: Gemma 4 model benchmark — run full theme extraction with each of the 4 downloaded models (`gemma4:e4b`, `VladimirGav/gemma4-26b-16GB-VRAM`, `gemma4:26b`, `gemma4:31b`) on the same input document. Measure wall-clock time and tok/s for each. Human quality review of outputs. Results inform which model to use per pipeline stage in model_settings presets.
- [ ] **TC-1d**: PolarQuant Gemma 4 31B GGUF — `caiovicentino1/Gemma-4-31B-it-PolarQuant-Q5` is SafeTensors only, not GGUF-compatible. Standard `convert_hf_to_gguf.py` won't preserve PolarQuant's custom quantization (hypersphere normalization + Hadamard rotation). Wait for a community GGUF conversion to appear on HuggingFace/Ollama, then add to models.toml and benchmark against `gemma4:31b`.
