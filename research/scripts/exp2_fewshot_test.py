"""Experiment 2: Few-Shot Example Count Test

Test how many few-shot examples are needed for reliable card generation.
Uses temperature 1.0 (winner from Experiment 1).

Generates 24 test cards at 4 different few-shot counts (0, 1, 3, 5 examples).
Scores all 96 cards on a 7-dimension rubric.

Usage:
    cd C:/Programming/MTGAI
    python research/scripts/exp2_fewshot_test.py
"""
...
FEWSHOT_DIR = Path("research/few-shot-examples")
...
# ---------------------------------------------------------------------------
# 24-Card Test Matrix (same as Exp 1)
# ---------------------------------------------------------------------------
...
# Few-shot settings
SETTINGS = [
    {"name": "2A", "num_examples": 0, "description": "Zero-shot — system prompt only"},
    {"name": "2B", "num_examples": 1, "description": "1 example per call — rarity match"},
    {"name": "2C", "num_examples": 3, "description": "3 examples — rarity + color + type match"},
    {"name": "2D", "num_examples": 5, "description": "5 examples — rarity + color + type + 2 diverse"},
]
...
def load_fewshot_library() -> list[dict]:
    """Load all few-shot example cards from the library."""
    index_path = FEWSHOT_DIR / "index.json"
    with open(index_path, "r", encoding="utf-8") as f:
        index = json.load(f)

    examples = []
    for entry in index:
        card_path = FEWSHOT_DIR / entry["filename"]
        if card_path.exists():
            with open(card_path, "r", encoding="utf-8") as f:
                card_data = json.load(f)
            # Merge index metadata with card data
            card_data["_rarity"] = entry["rarity"]
            card_data["_colors"] = entry["colors"]
            card_data["_card_type"] = entry["card_type"]
            card_data["_filename"] = entry["filename"]
            examples.append(card_data)

    return examples


def _get_majority_rarity(slot_indices: list[int]) -> str:
    """Get the most common rarity among the batch slots."""
    rarities = [CARD_SLOTS[i - 1]["rarity"] for i in slot_indices]
    from collections import Counter
    return Counter(rarities).most_common(1)[0][0]


def _get_batch_colors(slot_indices: list[int]) -> list[str]:
    """Get all unique single colors referenced by the batch."""
    colors = set()
...
def _get_batch_types(slot_indices: list[int]) -> list[str]:
    """Get all card types in the batch."""
    types = set()
    for idx in slot_indices:
        slot = CARD_SLOTS[idx - 1]
        # Normalize type: take the first word of the type (e.g., "Enchantment — Saga" -> "Enchantment")
        base_type = slot["type"].split(" —")[0].split(" -")[0].strip()
        # Also handle "Basic Land — Forest" -> "Land"
        if "Land" in slot["type"]:
            base_type = "Land"
        types.add(base_type)
    return list(types)
...
def select_examples(
    ...
    """Select few-shot examples for a batch.

    Strategy:
    - 1 example:  Pick one matching the majority rarity
    - 3 examples: 1 matching rarity + 1 matching color + 1 matching type
    - 5 examples: rarity + color + type + 2 diverse picks
    """
    if num_examples == 0:
        return []
    ...
    majority_rarity = _get_majority_rarity(slot_indices)
    batch_colors = _get_batch_colors(slot_indices)
    batch_types = _get_batch_types(slot_indices)

    def _pick_best(candidates: list[dict]) -> dict | None:
        """Pick the first candidate not already selected."""
        for c in candidates:
            if c["_filename"] not in used_filenames:
                return c
        return None

    # 1. Rarity match — always the first pick
    rarity_candidates = [e for e in library if e["_rarity"] == majority_rarity]
    if not rarity_candidates:
        # Fallback: any example
        rarity_candidates = library
    pick = _pick_best(rarity_candidates)
    if pick:
        selected.append(pick)
        used_filenames.add(pick["_filename"])

    if num_examples >= 3:
        # 2. Color match — pick an example sharing a color with the batch
        color_candidates = [
            e for e in library
            if any(c in e["_colors"] for c in batch_colors)
            and e["_filename"] not in used_filenames
        ]
        if not color_candidates:
            color_candidates = [e for e in library if e["_filename"] not in used_filenames]
        pick = _pick_best(color_candidates)
        if pick:
            selected.append(pick)
            used_filenames.add(pick["_filename"])

        # 3. Type match — pick an example of the same card type as one in the batch
        type_candidates = [
            e for e in library
            if any(bt.lower() in e["_card_type"].lower() for bt in batch_types)
            and e["_filename"] not in used_filenames
        ]
        if not type_candidates:
            type_candidates = [e for e in library if e["_filename"] not in used_filenames]
        pick = _pick_best(type_candidates)
        if pick:
            selected.append(pick)
            used_filenames.add(pick["_filename"])

    if num_examples >= 5:
        # 4-5. Diverse picks — fill remaining slots with different rarities/types
        remaining_needed = num_examples - len(selected)
        # Try to pick from different rarities than those already selected
        selected_rarities = {e["_rarity"] for e in selected}
        selected_types = {e["_card_type"] for e in selected}

        # First try diverse rarity
        diverse_candidates = [
            e for e in library
            if e["_rarity"] not in selected_rarities
            and e["_filename"] not in used_filenames
        ]
        if diverse_candidates:
            pick = _pick_best(diverse_candidates)
            if pick:
                selected.append(pick)
                used_filenames.add(pick["_filename"])
                remaining_needed -= 1

        # Then try diverse type
        if remaining_needed > 0:
            diverse_type_candidates = [
                e for e in library
                if e["_card_type"] not in selected_types
                and e["_filename"] not in used_filenames
            ]
            if diverse_type_candidates:
                pick = _pick_best(diverse_type_candidates)
                if pick:
                    selected.append(pick)
                    used_filenames.add(pick["_filename"])
                    remaining_needed -= 1
        ...
        while remaining_needed > 0:
            remaining = [e for e in library if e["_filename"] not in used_filenames]
            if not remaining:
                break
            pick = remaining[0]
            selected.append(pick)
    ...
    # Return only up to num_examples
    return selected[:num_examples]


def format_example_card(card: dict) -> str:
    """Format a card example as clean JSON (exclude internal metadata)."""
...
def build_batch_prompt(slot_indices: list[int], examples: list[dict]) -> str:
    """Build the user prompt for a batch of card slots, with optional few-shot examples."""
    lines = [
        f"Generate {len(slot_indices)} Magic: The Gathering cards. "
        "Each card must fill a specific slot.\n"
    ]

    # Add few-shot examples section if any
    if examples:
        lines.append("Here are examples of well-designed cards for reference:\n")
        for i, ex in enumerate(examples, 1):
            lines.append(f"Example {i}:")
            lines.append(format_example_card(ex))
            lines.append("")
        lines.append("Now generate cards for these slots:\n")
...
def score_card(card: dict, slot: dict) -> dict:
    ...
    if "Creature" in type_line and "\u2014" not in type_line and "—" not in type_line:
        tlc -= 1
    ...
    if slot_color not in ("-", "C"):
        expected_colors = list(slot_color)
        if colors:
...
def run_experiment():
    llm = CachedLLM()
    library = load_fewshot_library()
    print(f"Loaded {len(library)} few-shot examples from library")
    ...
    for setting in SETTINGS:
        ...
        num_examples = setting["num_examples"]
        print(f"\n{'='*60}")
        print(f"Setting {setting_name} ({num_examples} examples) — {setting['description']}")
        print(f"{'='*60}")
        ...
        for batch_idx, batch_slots in enumerate(BATCHES, 1):
            print(f"  Setting {setting_name} ({num_examples} examples), "
                  f"Batch {batch_idx}/{len(BATCHES)} (slots {batch_slots})...")

            # Select examples for this batch
            examples = select_examples(library, batch_slots, num_examples)
            if examples:
                ex_names = [e.get("name", "?") for e in examples]
                print(f"    Examples selected: {ex_names}")

            user_prompt = build_batch_prompt(batch_slots, examples)
            ...
            try:
                ...
                batch_data = {
                    "batch_num": batch_idx,
                    "slots": batch_slots,
                    "examples_used": [e.get("name", "?") for e in examples],
                ...
                setting_result["batches"].append(batch_data)
                ...
                for i, card in enumerate(cards):
                    ...
                    print(f"    Card {slot_num} '{card.get('name', '?')}': "
                          f"avg={avg:.1f}{fail_str}")
        ...
        all_raw_results.append(setting_result)
...
def generate_summary(raw_results: list, all_scores: list) -> str:
    """Generate a markdown summary comparing few-shot settings."""
    dimensions = [
    ...
    dim_labels = {
        "rules_text_correctness": "Rules Text Correctness",
        "mana_cost_appropriateness": "Mana Cost Appropriateness",
        "power_level_for_rarity": "Power Level for Rarity",
        "flavor_text_quality": "Flavor Text Quality",
        "name_creativity": "Name Creativity",
        "type_line_correctness": "Type Line Correctness",
        "color_pie_compliance": "Color Pie Compliance",
    }

    setting_names = [s["name"] for s in SETTINGS]

    lines = ["# Experiment 2: Few-Shot Example Count — Summary\n"]
    lines.append(f"**Model**: {MODEL}")
    lines.append(f"**Temperature**: {TEMPERATURE} (winner from Experiment 1)")
    lines.append(f"**Settings tested**: {', '.join(setting_names)}")
    ...
    for sr in raw_results:
        sname = sr["setting"]
        cost_by_setting[sname] = 0
        tokens_by_setting[sname] = {"input": 0, "output": 0}
        for b in sr["batches"]:
            c = b.get("cost_usd", 0)
            total_cost += c
            cost_by_setting[sname] += c
            tokens_by_setting[sname]["input"] += b.get("tokens", {}).get("input", 0)
            tokens_by_setting[sname]["output"] += b.get("tokens", {}).get("output", 0)

    lines.append(f"**Total API cost**: ${total_cost:.4f}\n")
    lines.append("---\n")

    # ---------------------------------------------------------------------------
    # Average scores per setting
    # ---------------------------------------------------------------------------
    lines.append("## Average Scores by Few-Shot Count\n")
    header = "| Dimension | 2A (0 ex.) | 2B (1 ex.) | 2C (3 ex.) | 2D (5 ex.) |"
    lines.append(header)
    lines.append("|" + "|".join(["-----------|" for _ in range(5)]))

    setting_dim_averages = {}
    for sname in setting_names:
        s_scores = [s for s in all_scores if s["setting"] == sname]
        setting_dim_averages[sname] = {}
        for dim in dimensions:
            vals = [s["scores"][dim] for s in s_scores if dim in s["scores"]]
            avg = sum(vals) / len(vals) if vals else 0
            setting_dim_averages[sname][dim] = avg

    for dim in dimensions:
        row = f"| {dim_labels[dim]} |"
        for sname in setting_names:
            row += f" {setting_dim_averages[sname][dim]:.2f} |"
        lines.append(row)
    ...
    setting_overall = {}
    for sname in setting_names:
        vals = list(setting_dim_averages[sname].values())
        avg = sum(vals) / len(vals) if vals else 0
        setting_overall[sname] = avg
        row += f" **{avg:.2f}** |"
    lines.append(row)
    lines.append("")

    # Best setting
    best_setting = max(setting_overall, key=setting_overall.get)
    lines.append(
        f"## Best Overall Setting: **{best_setting}** "
        f"(avg: {setting_overall[best_setting]:.2f})\n"
    )

    # ---------------------------------------------------------------------------
    # Key dimension: Rules Text Correctness comparison
    # ---------------------------------------------------------------------------
    lines.append("## Key Question: Do Few-Shot Examples Improve Rules Text Correctness?\n")
    lines.append("| Setting | Rules Text Correctness | Type Line Correctness | Delta vs 0-shot |")
    lines.append("|---------|----------------------|---------------------|-----------------|")
    baseline_rtc = setting_dim_averages["2A"]["rules_text_correctness"]
    baseline_tlc = setting_dim_averages["2A"]["type_line_correctness"]
    for sname in setting_names:
        rtc = setting_dim_averages[sname]["rules_text_correctness"]
        tlc = setting_dim_averages[sname]["type_line_correctness"]
        delta_rtc = rtc - baseline_rtc
        delta_sign = "+" if delta_rtc >= 0 else ""
        lines.append(
            f"| {sname} | {rtc:.2f} | {tlc:.2f} | "
            f"RTC {delta_sign}{delta_rtc:.2f} |"
        )
    lines.append("")
    ...
    lines.append("| Metric | 2A (0 ex.) | 2B (1 ex.) | 2C (3 ex.) | 2D (5 ex.) |")
    lines.append("|--------|-----------|-----------|-----------|-----------|")

    for label, dims in [
        ("Correctness (avg)", correctness_dims),
        ("Creativity (avg)", creativity_dims),
        ("Power Level", ["power_level_for_rarity"]),
    ]:
        row = f"| {label} |"
        for sname in setting_names:
            vals = [setting_dim_averages[sname][d] for d in dims]
            avg = sum(vals) / len(vals) if vals else 0
            row += f" {avg:.2f} |"
        lines.append(row)
    lines.append("")
    ...
    lines.append("| Transition | Overall Delta | RTC Delta | Creativity Delta |")
    lines.append("|-----------|---------------|-----------|------------------|")
    ...
    for label, from_s, to_s in transitions:
        overall_delta = setting_overall[to_s] - setting_overall[from_s]
        rtc_delta = (
            setting_dim_averages[to_s]["rules_text_correctness"]
            - setting_dim_averages[from_s]["rules_text_correctness"]
        )
        creat_delta = (
            sum(setting_dim_averages[to_s][d] for d in creativity_dims) / len(creativity_dims)
            - sum(setting_dim_averages[from_s][d] for d in creativity_dims) / len(creativity_dims)
        )
        sign_o = "+" if overall_delta >= 0 else ""
        sign_r = "+" if rtc_delta >= 0 else ""
        sign_c = "+" if creat_delta >= 0 else ""
        lines.append(
            f"| {label} | {sign_o}{overall_delta:.2f} | "
            f"{sign_r}{rtc_delta:.2f} | {sign_c}{creat_delta:.2f} |"
        )
    lines.append("")
    ...
    # ---------------------------------------------------------------------------
    lines.append("## Token Cost Comparison\n")
    lines.append(
        "Adding few-shot examples increases input tokens. "
        "Is the quality improvement worth the cost increase?\n"
    )
    lines.append("| Setting | Input Tokens | Output Tokens | Total Cost | Cost vs 0-shot |")
    lines.append("|---------|-------------|---------------|------------|----------------|")
    baseline_cost = cost_by_setting.get("2A", 0)
    for sname in setting_names:
        inp = tokens_by_setting.get(sname, {}).get("input", 0)
        out = tokens_by_setting.get(sname, {}).get("output", 0)
        cost = cost_by_setting.get(sname, 0)
        if baseline_cost > 0:
            multiplier = cost / baseline_cost
            cost_note = f"{multiplier:.1f}x"
        else:
            cost_note = "baseline"
        lines.append(
            f"| {sname} ({SETTINGS[setting_names.index(sname)]['num_examples']} ex.) | "
            f"{inp:,} | {out:,} | ${cost:.4f} | {cost_note} |"
        )
    lines.append("")

    # ---------------------------------------------------------------------------
    # Failure modes per setting
    # ---------------------------------------------------------------------------
    lines.append("## Failure Modes by Setting\n")
    for sname in setting_names:
        s_scores = [s for s in all_scores if s["setting"] == sname]
        all_failures = {}
        for s in s_scores:
        ...
        if all_failures:
            ...
            num_ex = SETTINGS[setting_names.index(sname)]["num_examples"]
            lines.append(f"### {sname} ({num_ex} examples)")
    ...
    # ---------------------------------------------------------------------------
    lines.append("## Per-Card Scores (All Settings)\n")
    lines.append(
    ...
    for sname in setting_names:
        s_scores = sorted(
            [s for s in all_scores if s["setting"] == sname],
        ...
        for s in s_scores:
            ...
            cname = s.get("card_name", "?")
            if len(cname) > 22:
                cname = cname[:19] + "..."
            lines.append(
                f"| {s['card_slot']:2d} | {sname} | {cname} | "
    ...
    best_correct_setting = max(
        setting_names,
        key=lambda s: sum(setting_dim_averages[s][d] for d in correctness_dims) / len(correctness_dims),
    )
    best_creative_setting = max(
        setting_names,
        key=lambda s: sum(setting_dim_averages[s][d] for d in creativity_dims) / len(creativity_dims),
    )

    correctness_avg = sum(
        setting_dim_averages[best_correct_setting][d] for d in correctness_dims
    ) / len(correctness_dims)
    creativity_avg = sum(
        setting_dim_averages[best_creative_setting][d] for d in creativity_dims
    ) / len(creativity_dims)

    lines.append(
        f"- **Best setting for correctness**: {best_correct_setting} "
        f"(avg correctness: {correctness_avg:.2f})"
    )
    lines.append(
        f"- **Best setting for creativity**: {best_creative_setting} "
        f"(avg creativity: {creativity_avg:.2f})"
    )
    lines.append(
        f"- **Best overall setting**: {best_setting} "
        f"(overall avg: {setting_overall[best_setting]:.2f})"
    )
    lines.append("")

    # Diminishing returns note
    delta_0_to_3 = setting_overall.get("2C", 0) - setting_overall.get("2A", 0)
    delta_3_to_5 = setting_overall.get("2D", 0) - setting_overall.get("2C", 0)

    lines.append("### Diminishing Returns Verdict\n")
    if abs(delta_3_to_5) < abs(delta_0_to_3) * 0.3:
        lines.append(
            "The jump from 3 to 5 examples shows significant diminishing returns compared to "
            "0 to 3 examples. **3 examples is the sweet spot** for cost-effectiveness."
        )
    elif delta_3_to_5 > delta_0_to_3:
        lines.append(
            "Quality continues to improve meaningfully from 3 to 5 examples. "
            "**5 examples is recommended** if the token budget allows."
        )
    else:
        lines.append(
            f"0->3 improved scores by {delta_0_to_3:+.2f}, while 3->5 improved by "
            f"{delta_3_to_5:+.2f}. The marginal gain is smaller, suggesting "
    ...
    if baseline_cost > 0:
        cost_3 = cost_by_setting.get("2C", 0)
        cost_5 = cost_by_setting.get("2D", 0)
        lines.append(
            f"- 0-shot baseline cost: ${baseline_cost:.4f} for 24 cards"
        )
        lines.append(
            f"- 3 examples cost: ${cost_3:.4f} ({cost_3/baseline_cost:.1f}x baseline)"
        )
        lines.append(
            f"- 5 examples cost: ${cost_5:.4f} ({cost_5/baseline_cost:.1f}x baseline)"
        )
        lines.append(
            f"- For a full 280-card set, the cost difference would be approximately "
            f"${(cost_3 - baseline_cost) * (280/24):.2f} more with 3 examples vs 0-shot."
        )
    lines.append("")

    # Final recommendation
    lines.append(f"### Final Recommendation for Production Use\n")
    lines.append(
        f"**Recommended few-shot count for Phase 1C**: "
        f"**{SETTINGS[setting_names.index(best_setting)]['num_examples']} examples** "
        f"(Setting {best_setting})\n"
    )
    lines.append(
        "Use the matching strategy: pick examples by rarity, color, and type to maximize "
        "relevance to the cards being generated."
    )
